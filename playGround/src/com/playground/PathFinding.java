...
public class PathFinding {
	...
	public void init(TiledMap map) {
		...
		tiles = (TiledMapTileLayer)map.getLayers().get("poop");
		
		Iterator<TiledMapTile> it = map.getTileSets().getTileSet("tiles").iterator();
		while(it.hasNext()) {
...
//	}
	...
	private Cell getCellAt(float x, float y) {
		float pixelX = x * 64.0f;
		float pixelY = y * 64.0f;
	...
	private void processRequest() {
		...
		initialNavCell.g = 0;
		initialNavCell.h = h(initialNavCell); //just like calculatePath method in yours.
	...
	private void recoverPath(NavCell current) {
		...
		float x = (current.x * width + width * 0.5f) * 64.0f;
		float y = (current.y * height + height * 0.5f) * 64.0f;
